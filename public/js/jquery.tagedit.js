	...
	$.fn.tagedit = function(options) {
		...
		*/
		function inputsToList() {
			...
			// RH - deleted "disabled" att & class - not necessary AFAIK and kills tabindex
			// need to add tabindex as a variable that can be passed in
			html += '<input type="text" name="'+baseName+'[]" value="" id="tagedit-input" tabindex="'+options.tabIndex+'" dir="'+options.direction+'"/>';
			html += '</li>';
			html += '</ul>';
			...
			elements
				...
				.find('#tagedit-input')
					.each(function() {
						...
						// Event ist triggered in case of choosing an item from the autocomplete, or finish the input
						...
						.blur(function() {
							if($(this).val().length == 0) {
								...
								// RH - disabled - surely this (ie empty fields) should be handled server-side? 
